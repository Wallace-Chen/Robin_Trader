...
from execute_orders_simulation import *
...
#			thread_execute = orderExecution(self.stk,  self.opt, self.stk.period_ma1, self.stk.period_ma2, self.stk.period_ma3, self.refresh_rate, 200)
#			thread_execute.start()
			while True:
				time.sleep(self.refresh_rate)
				r.strategy_ema(self.stk, self.opt, self.send_pipe, self.send_pipe_opt)
				
				print("type: ",self.opt.type)
				if((not initOpt) and (not self.opt.type == "") and len(self.opt.price)==0):
					print("option will be initialized!")					
					initOpt = True
					initializeOption_simulation(self.stk, self.opt)
...
if __name__ == '__main__':
	...
	myStrategy = tradingStrategy("AAPL",1, 0.05, 0.1, 0.2, True)
	myStrategy.run()
